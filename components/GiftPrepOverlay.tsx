...
export function GiftPrepOverlay({
  ...
  return (
    <div className="absolute inset-0 z-30 flex items-center justify-center bg-black/55 p-4 md:p-6">
      <div className="moonlit-frame max-h-full w-full max-w-4xl overflow-y-auto rounded-[30px] border border-white/10 px-4 py-5 pb-safe md:px-8 md:py-8">
        <p className="text-xs uppercase tracking-[0.3em] text-sakura/80 md:text-sm">{prep.eyebrow}</p>
        <h2 className="mt-3 text-2xl font-semibold text-paper-cream md:text-4xl">{prep.title}</h2>
        <p className="mt-3 text-lg italic text-paper-cream/80 md:text-xl">{prep.subtitle}</p>
        <p className="mt-5 max-w-3xl text-base leading-8 text-paper-cream/78">
          {prep.description}
        </p>
        ...
        <div className="mt-6 rounded-[24px] border border-lantern-gold/18 bg-lantern-gold/8 px-5 py-4">
          ...
          <p className="mt-3 text-lg leading-8 text-paper-cream">{prep.simbaLine}</p>
        </div>
        ...
        <div className="mt-6 grid gap-4 md:grid-cols-2">
          ...
          <div className="grid gap-4">
            <div className="rounded-[24px] border border-sky-300/15 bg-sky-300/8 p-4 md:p-5">
              ...
              <p className="mt-3 text-lg leading-8 text-paper-cream">{prep.focusText}</p>
